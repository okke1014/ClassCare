...
export function CalendarWidget() {
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-white">
        <div className="flex items-center gap-2">
            ...
            <button
              ...
              aria-label={view === 'month' ? "Switch to week view" : "Switch to month view"}
        ...
        <div className="flex gap-1">
          <button onClick={prev} aria-label="Previous" className="p-1.5 hover:bg-gray-100 rounded-full">
            <ChevronLeft className="w-4 h-4" />
          </button>
          <button onClick={next} aria-label="Next" className="p-1.5 hover:bg-gray-100 rounded-full">
      ...
      {view === 'month' && (
          <>
          <div className="px-3 pb-3 flex-shrink-0 transition-all duration-300 overflow-hidden">
            ...
            <div className="grid grid-cols-7 gap-y-1">
                ...
                return (
                    ...
                    className={cn(
                        "flex flex-col items-center p-1 relative rounded-lg transition-colors",
            ...
            <div className="space-y-3">
                {selectedEvents.length === 0 ? (
                    <EmptyState
                        ...
                        description="Pick another date to see its schedule."
                    />
                ) : (
                    selectedEvents.map(event => (
                        <div key={event.id} className={cn(
                            ...
                            <button
                                type="button"
                                disabled={event.status === 'absent'}
                                className={cn(
                                    "flex-1 p-1 rounded-lg text-left min-w-0",
                                    event.status !== 'absent' && "hover:bg-gray-50"
                                )}
        ...
        <div className="flex flex-col flex-1 overflow-hidden relative">
            <div className="flex-1 overflow-y-auto relative">
                <div className="sticky top-0 z-30 grid grid-cols-[4rem_1fr_1fr_1fr_1fr_1fr_1fr_1fr] border-b bg-gray-50 shadow-sm">
                    <div className="p-2 border-r bg-gray-100 text-[11px] text-gray-500 text-center font-medium">
                      Period
                    </div>
                ...
                <div className="grid grid-cols-[4rem_1fr_1fr_1fr_1fr_1fr_1fr_1fr]" style={{ minHeight: `${DISPLAY_PERIODS.length * 50}px` }}>
                    <div className="border-r bg-gray-50 sticky left-0 z-20">
                        {DISPLAY_PERIODS.map(period => (
                            <div key={period.period} className="h-[50px] text-[11px] text-gray-500 px-1 py-1 border-b flex flex-col justify-center">
                                ...
                                {dayEvents.map(event => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            onClick={() => event.status !== 'absent' && router.push(`/class/${event.id}`)}
                                            className={cn(
                                                "absolute inset-x-0.5 rounded-lg p-1 text-[11px] text-left overflow-hidden shadow-sm border-l-2 opacity-90 z-10 transition-all",
                                                event.status !== 'absent' && 'hover:brightness-95',
